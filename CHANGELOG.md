...
- **CRITICAL — daily loss limit was silently disabled** (`services/risk_engine/engine.py`) — `_get_todays_pnl()` called `get_redis()` without importing it. The resulting `NameError` was swallowed by a broad `except Exception` that fail-opened to `return 0.0`, so with any position open the daily loss limit never fired — reproduced with a stubbed DB returning a ₹-99,999 loss. Fixed: `get_redis` is now imported correctly, and every failure path in `_get_todays_pnl()` fails **closed** — a DB read failure returns `float("-inf")` (halts trading) instead of `0.0` (allows it), matching the fail-closed behaviour already used by every other `RiskEngine` DB helper (`_get_open_count`, `_has_open_position`, `_get_open_notional`, `_has_traded_today`).

- **API authentication added for mutating endpoints** (`api/main.py`, `config/settings.py`, `.env.example`) — The API had no auth on state-changing routes while the `Procfile` binds `--host 0.0.0.0`, exposing them to the network. Added a new `settings.api_key` (`API_KEY` in `.env`), enforced via a `require_api_key` FastAPI dependency that checks the `X-API-Key` header with `secrets.compare_digest`. Applied to `POST /api/config` and `POST /api/bot/square-off`. In `live`/`semi-auto` (real broker) an unset `API_KEY` now hard-refuses mutating requests with 401 rather than allowing unauthenticated writes to reach a real-money broker; in `dev`/`paper` it's optional (logs a one-time warning) so the local dashboard keeps working with zero config.

### Fixed

- **Config endpoint ignored its own min/max** (`config/bot_config.py`) — `set_bot_config()` validated that keys existed in `CONFIG_SCHEMA` but never enforced the `min`/`max` declared there, so out-of-range values (e.g. `confidence_threshold=-999`) were persisted straight to Redis. `set_bot_config()` now rejects updates outside the schema's declared bounds; `get_bot_config()` also falls back to the schema default (with a warning log) if a value already sitting in Redis is out of range.

- **`pytest`/`pytest-asyncio` missing from `requirements.txt`** — `make test` (`pytest tests/ -v`) depended on a package the project never declared; a clean `make setup` could not run the suite at all. Added `pytest==9.1.1` and `pytest-asyncio==1.4.0` under a new "Testing" section. `tests/test_charges.py`, `tests/test_lifecycle_math.py`, and `tests/test_risk_engine.py` were also repaired to match current `RiskEngine`/lifecycle behaviour (stale assertions from before the ATR-multiplier and fail-closed changes above).

- **"Square Off All" did not square off** (`api/main.py`) — `POST /api/bot/square-off` only called the broker when a real broker was configured, so in `dev`/`paper` it emitted a WebSocket event and closed nothing; in `live`/`semi-auto` it closed positions at the broker without reconciling the DB, leaving trades stuck at `status='OPEN'`. Now calls the broker (when real) **and** reconciles via `TradeLifecycleManager.close_all_open_trades()` in every mode; the response reports how many trades were closed and surfaces any broker-side error without blocking the DB reconciliation. Scope is **intraday-only**, matching the dashboard button's label: `ZerodhaOrderManager.square_off_all_intraday()` filters to `product == "MIS"` and never exits CNC, so closing SWING trades in the DB here would mark them `CLOSED` while the real CNC position stayed open at the broker. `close_all_open_trades()` gained an explicit `intraday_only` parameter so scope no longer has to be inferred from the exit-reason string.

- **Intraday trailing-stop schedule was dead code** (`services/execution/trade_lifecycle.py`) — `_update_trailing_stop()` selected its milestone schedule via `trade.get("timeframe", "1day")`, but the `trades` table has no `timeframe` column (it has `strategy_mode`, and `_load_open_trades()` selects exactly that). The `.get()` therefore always fell through to `"1day"`, making `is_swing` permanently `True`: every intraday trade (ORB, 5-min box, IDARVAS) silently used the SWING schedule (no trail until 4R) instead of the intended INTRADAY one (breakeven at 2R, +1R at 3R, +2R at 5R). Now reads `strategy_mode`, with a missing/NULL value treated as INTRADAY to match the intraday filter already used in `_load_open_trades()`.

- **Paper broker could fill at ₹0** (`services/execution/paper_broker.py`) — When a symbol's tick was missing from Redis, `_get_market_price()` returned `0.0` and `place_order()` recorded a `COMPLETE` order at price 0, silently corrupting paper P&L. `place_order()` now refuses to simulate a fill when no market price is available and returns `None`; the caller treats this as a failed entry and marks the trade `CANCELLED`.
