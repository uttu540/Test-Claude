...
# Max hold on daily TF: 35 trading days (~7 calendar weeks)
# MFE analysis showed MAX_HOLD at 20d was force-exiting running trades
...
class MomentumTrade:
    ...
    rsi:              float       = 0.0
    adx:              float       = 0.0
    # Max favorable excursion — highest price reached during the trade
    # Used to diagnose: did the trade work before reversing, or go straight down?
    max_price:        float       = 0.0
    max_gain_pct:     float       = 0.0   # (max_price - entry) / entry * 100
...
class MomentumBacktestEngine:
    ...
    def __init__(
        ...
        watchlist_filter:      bool  = False,  # RS-ranked incubator filter (top-50 per day)
        watchlist_size:        int   = 50,     # how many symbols qualify each day
        ranging_sector_roc:    float = 5.0,    # min sector ROC-20% to trade RANGING
        ranging_rs_threshold:  float = 3.0,    # min stock RS vs Nifty to trade RANGING
    ) -> None:
        ...
        self._ranging_sector_roc    = ranging_sector_roc
        self._ranging_rs_threshold  = ranging_rs_threshold

        # RS watchlist filter (opt-in)
        # _watchlist_by_date[d] = set of symbols that would have been in top-N RS watchlist
        self._watchlist_filter    = watchlist_filter
        self._watchlist_size      = watchlist_size
        self._watchlist_by_date:  dict[date, set[str]] = {}   # populated in run()
    ...
    async def run(self) -> MomentumBacktestResult:
        ...
        # Build historical RS watchlist if filter enabled.
        # Batch-fetches all symbol daily data, scores RS per trading day,
        # produces dict[date → set[symbol]] for O(1) gate in _backtest_symbol.
        if self._watchlist_filter:
            log.info("momentum_bt.watchlist_build_start", symbols=len(self._symbols))
            self._watchlist_by_date = await self._build_historical_watchlist()
            log.info(
                "momentum_bt.watchlist_built",
                dates=len(self._watchlist_by_date),
                avg_size=round(
                    sum(len(v) for v in self._watchlist_by_date.values())
                    / max(len(self._watchlist_by_date), 1), 1
                ),
            )
    ...
    async def _backtest_symbol(self, symbol: str) -> list[MomentumTrade]:
        ...
        for i in range(len(trade_df)):
            ...
            # ── Gate 0: RS Watchlist filter ───────────────────────────────────
            # When enabled, only process days where this symbol is in the top-N
            # RS-ranked watchlist for that specific date. This mirrors the live
            # engine's behaviour: WatchlistBuilder.update() runs daily at 4 PM
            # and filters the universe to ~50 leaders before signal detection.
            if self._watchlist_filter and self._watchlist_by_date:
                if symbol not in self._watchlist_by_date.get(candle_date, set()):
                    continue
            ...
            if nifty_regime == "RANGING":
                if self._watchlist_filter:
                    # Watchlist filter: top-50 RS stocks unblocked in RANGING.
                    pass
                elif self._enable_sector_filter:
                    # RANGING + sector tailwind = sector rotation play.
                    # Nifty flat but SECTOR trending up → genuine edge.
                    # Logic: defense/sugar/capital markets make new highs while
                    # index chops sideways. Same Darvas setup, different context.
                    _sector = self._symbol_to_sector.get(symbol)
                    if not _sector:
                        continue   # unmapped sector — can't confirm tailwind
                    _sector_roc = self._sector_roc_by_date.get(_sector, {}).get(candle_date)
                    if _sector_roc is None or _sector_roc < self._ranging_sector_roc:
                        continue   # sector ROC below threshold — no sector trend
                    # Also require stock to be outperforming Nifty (sector rotation leader)
                    _rs_threshold = self._ranging_rs_threshold
                else:
                    # Default: block RANGING entirely (23% WR on all stocks historically).
                    continue
            ...
            # Gap-up >3% blocked 237 trades — these are strong continuation moves,
            # NOT chasing. Filtering them dropped PnL from ₹7.6L→₹4.4L. Gap-down
            # filter only hit 3 trades (negligible). Net: don't filter on open gap.
            ...
            # P5: Regime-specific hold + stop calibration.
            # TRENDING_UP: tight stop (1.5×ATR), 20-day hold — broad tailwind.
            # RANGING: wide stop (2×ATR), 45-day hold — sector rotation themes
            #          run for months; 20-day hold exits too early.
            # TRENDING_DOWN: wide stop (2×ATR), 40-day hold — genuine leaders only.
            if nifty_regime == "TRENDING_UP":
                _stop_atr_mult  = 1.5
                _trade_max_hold = MAX_HOLD_DAYS   # 20 days
            elif nifty_regime == "RANGING":
                _stop_atr_mult  = 2.0
                _trade_max_hold = 45              # sector rotation: themes last months
            else:  # TRENDING_DOWN
                _stop_atr_mult  = 2.0
    ...
    def _simulate_exit(
        ...
        exit_reason   = "OPEN"
        hold          = 0
        max_price     = entry_price   # highest price reached during trade (MFE)
        ...
        for idx, (ts, candle) in enumerate(future_df.iterrows()):
            ...
            # Trailing stop milestones — loosened based on MFE analysis.
            # Old: 2R → breakeven, 3R → +1R  (too aggressive — cut TORNTPOWER at 3.6% from 18.9%)
            # New: first trail only at 4R → +1R, giving big movers room to breathe.
            # Momentum stocks have volatile pullbacks mid-move; early trail = killed winners.
            if initial_risk > 0:
                move       = h - entry_price
                ...
                if r_multiple >= 12:
                    ...
                    new_trail = entry_price + 1 * initial_risk  # first lock-in at 4R
                else:
                    new_trail = stop_loss   # no trail below 4R — let trade breathe
    ...
    async def _build_historical_watchlist(self) -> dict[date, set[str]]:
        """
        Simulate what the RS-ranked watchlist would have contained on every trading day.

        For each date d in [start, end]:
          1. Each symbol's RS score = stock_20d_roc(d) - nifty_20d_roc(d)
          2. Quality gates: price ≥ 100, avg_vol ≥ 200k, ≤15% below 52wk high,
             not up >50% in prior 60 days
          3. Top-N by RS → added to watchlist_by_date[d]

        Uses batch yfinance download (100 symbols per batch) for speed.
        Returns dict[date → frozenset of qualifying symbols].
        """
        RS_LOOKBACK    = 20
        MIN_PRICE      = 100.0
        MIN_AVG_VOL    = 200_000
        MAX_52W_DROP   = 0.15   # must be within 15% of 52wk high
        MAX_PRETREND   = 0.50   # skip if up >50% in prior 60 days

        load_start = self._start - timedelta(days=300)  # warmup for 20d ROC + 52wk high
        load_end   = self._end

        # Batch-fetch all symbol daily data
        ...
        for i in range(0, len(tickers), batch_size):
            ...
            try:
                ...
                log.warning("momentum_bt.watchlist_batch_error", batch=i, error=str(e))

        log.info("momentum_bt.watchlist_data_loaded", loaded=len(all_close))

        if not all_close:
            return {}

        # Build aligned price matrix (date × symbol)
        close_df  = pd.DataFrame(all_close).sort_index()
        high_df   = pd.DataFrame(all_high).sort_index()
        volume_df = pd.DataFrame(all_volume).sort_index()

        # Trading days in backtest range
        trading_days = [
            d for d in close_df.index.date
            if self._start <= d <= self._end
        ]

        watchlist_by_date: dict[date, set[str]] = {}

        for d in trading_days:
            d_ts   = pd.Timestamp(d)
            nifty_roc20 = self._nifty_roc20_by_date.get(d, 0.0)

            scored: list[tuple[str, float]] = []

            for sym in all_close:
                try:
                    # Slice data up to and including date d (point-in-time safe)
                    c_series = all_close[sym]
                    c_up_to  = c_series[c_series.index <= d_ts]
                    if len(c_up_to) < RS_LOOKBACK + 5:
                        continue

                    last_close = float(c_up_to.iloc[-1])
                    if last_close < MIN_PRICE:
                        continue

                    # Volume gate
                    v_series = all_volume[sym]
                    v_up_to  = v_series[v_series.index <= d_ts]
                    avg_vol  = float(v_up_to.iloc[-20:].mean()) if len(v_up_to) >= 20 else 0.0
                    if avg_vol < MIN_AVG_VOL:
                        continue

                    # 52-week high proximity
                    h_series = all_high[sym]
                    h_up_to  = h_series[h_series.index <= d_ts]
                    high_52w = float(h_up_to.iloc[-min(252, len(h_up_to)):].max())
                    if high_52w > 0 and (high_52w - last_close) / high_52w > MAX_52W_DROP:
                        continue

                    # Overextension check
                    if len(c_up_to) >= 60:
                        base60 = float(c_up_to.iloc[-60])
                        if base60 > 0 and (last_close - base60) / base60 > MAX_PRETREND:
                            continue

                    # RS score = stock 20d ROC - Nifty 20d ROC
                    base_roc = float(c_up_to.iloc[-(RS_LOOKBACK + 1)])
                    stock_roc20 = (last_close - base_roc) / base_roc * 100 if base_roc > 0 else 0.0
            ...
            watchlist_by_date[d] = {sym for sym, _ in scored[: self._watchlist_size]}

        return watchlist_by_date
